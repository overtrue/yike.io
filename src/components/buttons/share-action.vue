<template>
  <div class="share-action">
    <relation-btn relation="thread" action="subscribe" :item="item">
      ...
      <button class="mx-auto mb-2 btn btn-icon text-22" slot="off"
              :class="{'btn-tiffany': !hovering, 'btn-danger': hovering}"
              @mouseenter="hovering=true"
              @mouseleave="hovering=false" title="取消订阅">
        <volume-mute v-if="hovering" title="取消订阅"></volume-mute>
        <check-icon v-else></check-icon>
      </button>
    </relation-btn>
    <button class="mx-auto mb-2 btn btn-icon text-22 copy-link text-gray-50" :data-clipboard-text="copyText" title="复制链接"><link-variant title="复制链接" /></button>
