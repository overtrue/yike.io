          ...
          <header class="thread-header d-flex justify-content-between align-items-center">
            ...
            <div class="thread-actions">
              ...
              <div class="dropdown-menu dropdown-menu-right">
                <button class="dropdown-item" type="button" @click="$router.push({name:'threads.edit', params:{id: thread.id}})"><pencil-icon></pencil-icon> 编辑</button>
                <button class="dropdown-item" type="button"><delete-icon></delete-icon> 删除</button>
              </div>
            </div>
          </header>
...
<script>
  ...
  export default {
    ...
    methods: {
      loadThread() {
        this.api('threads').find(this.$route.params.id).then(response => this.thread = response).then(() => {
          this.$nextTick(() => {
            setTimeout(() => {
              Prism.highlightAll()
            }, 300)
